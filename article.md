...
`ReverseSshServerPort` - порт на который мы будем устанавливать соединение до моего домашнего RPI. Именно этот порт и надо добавить в правила Port Forwarding. У сисадминов бытует мнение, что для ssh всенда надо выбирать нестандартный порт (отличный от 22), т.к. это спасает от обнаружения массовым сканированием. Я не сисадмин, но имейте ввиду - этот порт будет торчать наружу из вашей **домашней** сети.
...
Осталось немного полирнуть - сделать так, чтобы он теперь сам, без моих манипуляций, соединялся со мной, когда это возможно. Тут нам снова поможет подсистема сервисом\демонов (простите я всегда путаюсь в этой терминологии) `systemctl`. Я уже в первой статье создавал такой сервис - тот, который запускает наш логгер RpiProbeLogger. Тут механика та же, нам потребуется `.service`-файл:
